...
function MainQuestion() {
  ...
  useEffect(() => {
    async function getFunctionDetails() {
      await axios
        .get(`https://sof-clone-app1.herokuapp.com/api/question/${id}`)
  ...
  const handleSubmit = async () => {
    ...
    await axios
      .post("https://sof-clone-app1.herokuapp.com/api/answer", body, config)
      .then(() => {
  ...
  const handleComment = async () => {
    if (comment !== "") {
      ...
      await axios.post(`https://sof-clone-app1.herokuapp.com/api/comment/${id}`, body).then((res) => {
        setComment("");
        setShow(false);
        getUpdatedAnswer();
                  ...
                  {questionData?.comments &&
                    questionData?.comments.map((_qd) => (
                      <p key={_qd?._id}>
                        {_qd.comment}{" "}
                        <span>
                          - {_qd.user ? _qd.user.displayName : "James Scott"}
                        </span>{" "}
                        {"    "}
                        <small>
